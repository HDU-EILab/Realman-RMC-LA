...
class HandDebugGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Realman 灵巧手 · 升降 · 机械臂 · 底盘 调试")
        self.root.geometry("940x720")
        self.root.minsize(820, 560)

        self.host_var = tk.StringVar(value="192.168.10.18")
    ...
    def _build_ui(self):
        ...
        ttk.Label(top, text="控制器IP").grid(row=0, column=0, padx=6, pady=6, sticky="w")
        ttk.Entry(top, textvariable=self.host_var, width=18).grid(row=0, column=1, padx=6, pady=6)
        ttk.Label(top, text="端口").grid(row=0, column=2, padx=6, pady=6, sticky="w")
        ttk.Entry(top, textvariable=self.port_var, width=8).grid(row=0, column=3, padx=6, pady=6)

        btn_connect = ttk.Button(top, text="连接", command=self.connect)
        btn_connect.grid(row=0, column=4, padx=6, pady=6)
        btn_disconnect = ttk.Button(top, text="断开", command=self.disconnect)
        btn_disconnect.grid(row=0, column=5, padx=6, pady=6)
        btn_get_state = ttk.Button(top, text="读机械臂状态", command=self.get_arm_state)
        btn_get_state.grid(row=0, column=6, padx=6, pady=6)
        btn_log_window = ttk.Button(top, text="打开日志窗口", command=self.open_log_window)
        btn_log_window.grid(row=0, column=7, padx=6, pady=6)
        ...
        ttk.Label(
            top,
            text="下方为分页；指令表见 INTERFACE_REFERENCE.md（或 SYSTEM_CONTROL_REFERENCE.md）",
            foreground="#444",
        ).grid(row=1, column=3, columnspan=5, padx=8, pady=4, sticky="w")
        ...
        self.log.insert(
            "end",
            "提示: 使用上方分页切换功能区；指令汇总见 INTERFACE_REFERENCE.md / SYSTEM_CONTROL_REFERENCE.md；"
            "底盘 SSH 需 pip install paramiko，主控须 roslaunch agv_ros agv_start.launch。\n",
        )

        # Button tooltips: show command and key params.
        ToolTip(btn_connect, "连接到控制器\n参数: host=控制器IP, port=控制器端口")
        ToolTip(btn_disconnect, "断开当前TCP连接")
        ToolTip(btn_get_state, "读取机械臂当前状态\n命令: get_current_arm_state")
        ...
        ToolTip(btn_joint_deg, "TCP JSON: {\"command\":\"get_joint_degree\"}")
            ...
            try:
                ...
                if self.client is not None:
                    try:
                        ...
                        pass
                    self.client = None
                cli = RealmanHandClient(host, port)
                cli.connect()
                self.client = cli
                cli = None
                self._log(f"[OK] 已连接 {host}:{port}")
        ...
        def work():
            try:
                if not self.client:
                    ...
                    return
                self._send_and_log({"command": "get_current_arm_state"}, wait_ms=400)
            except Exception as e:
        ...
        def work():
            try:
                if not self.client:
                    self._bg_warn_not_connected()
                    return
                self._send_and_log({"command": "get_joint_degree"}, wait_ms=500)
            except Exception as e:
        ...
        def work():
            try:
                if not self.client:
                    self._bg_warn_not_connected()
                    return
                self._send_and_log({"command": "clear_system_err"}, wait_ms=500)
            except Exception as e:
        ...
        def work():
            try:
                if not self.client:
                    self._bg_warn_not_connected()
                    return
                self._send_and_log({"command": "set_arm_stop"}, wait_ms=600)
            except Exception as e:
